...
export class MapCrtl extends Component {
    ...
    index: number = 0
    initialized = false;
    tiles = ["踏上旅途", "冲向妖界", "龙宫探宝", "地府改命", "大闹天宫","西天取经","修成正果","血色禁地"]
    start() {
        this.Title.getComponent(Label).string = "踏上旅途 1/8"
        this.refresh()
    }
    onEnable() {
        if (!this.initialized) {
    ...
    refresh() {
        ...
        if (
            ...
            currentChapter < 1 || currentChapter > 8 ||
    ...
    right() {
        ...
        this.Title.getComponent(Label).string = this.tiles[this.index] + (this.index + 1) + "/8"
