import { CCInteger, Color, Component, Enum, Graphics, SpriteFrame, UITransform, _decorator, Node, Size, Sprite, isValid } from "cc";
...
export class QRCode extends Component {
	...
	@property({type: SpriteFrame, tooltip: "中间显示的icon"})
	...
	set foreground(value: Color) {
		this.Foreground = value;
	}

	@property({ type: CCInteger, tooltip: "二维码外边距"})
	...
	private initGraphics() {
		...
		this.Background = this.Background || Color.WHITE;
		this.Foreground = this.Foreground || Color.BLACK;
		if (!this._graphics) {
	...
	public drawQRCode(options: QRCodeOptions): void {
		...
		options.background = options.background || Color.WHITE;
		options.foreground = options.foreground || Color.BLACK;
		...
		if (options.icon) {
			...
			let uiTransform = iconNode.getComponent(UITransform) || iconNode.addComponent(UITransform);
			uiTransform.setContentSize(new Size(qrcWidth / 5, qrcHeight / 5));
