        ...
        fetch(config.ServerUrl.url + "queryBeRobLog", options)
            ...
            .then(async data => {
                ...
                for (let i = 0; i < messageDetails.length; i++) {
                    ...
                    let content = null;
                    content = `<color=#E36F1A>${messageDetail.timeStr}  <color=#EEE365>我 </color>在<color=#EEE365> 矿场夺取中</color>遭到<color=#EEE365>${messageDetail.nickname}</color>抢夺你<color=#00BCD4>${messageDetail.robSilver}金币</color>。</color>`
