...
@ccclass('kuanJinjiCtrl')
export class kuanJinjiCtrl extends Component {
    @property(Node)
    WinCount: Node
    @property(Node)
    GameRanking: Node
    ...
    public refushData() {
        ...
        fetch(config.ServerUrl.url + "kuanList", options)
            .then(response => {
                if (!response.ok) {
            ...
            .then(async data => {
                ...
                if (data.success == '1') {
                    ...
                    for (let i = 0; i < userlist.length; i++) {
                        ...
                        item.getChildByName("itemCount").getComponent(Label).string = "当前银两：" + userlist[i].currentSilver 
    ...
    public async clickTiaozhanFun(userId) {
        ...
        fetch(config.ServerUrl.url + "battle7", options)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
