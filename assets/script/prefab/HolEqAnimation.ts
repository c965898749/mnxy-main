import { _decorator, AssetManager, Component, dragonBones, Node, resources, sp, Sprite, SpriteFrame } from 'cc';
...
@ccclass('HolEqAnimation')
export class HolEqAnimation extends Component {

    // // 动画速度
    public get timeScale() {
        if (this.$animationControl instanceof sp.Skeleton || this.$animationControl instanceof dragonBones.ArmatureDisplay) {
    ...
    async initBones(animationOption: HolAnimationOption) {
            ...
            this.node.getComponent(Sprite).spriteFrame =
                await util.bundle.load(animationOption.avatarPath, SpriteFrame)
