...
export class Character extends CharacterMetaState {
    ...
    苦痛箭 Lv{skillLv}
    场下每回合对同位置敌人造成{healVal}点真实伤害
    `.replace(/ /ig, "")

    PassiveIntroduceTwo: string = `
    
    虹吸打击 Lv{skillLv}
    攻击时有{healVal}%几率进行87%吸血
    `.replace(/ /ig, "")

    SkillIntroduce: string = `
    
    玄冥协同 Lv{skillLv}
    与玄冥在同一队伍时，增加自身{healVal}点生命上限，{healVal2}点攻击，{healVal3}点速度
    `.replace(/ /ig, "")
    ...
    public getSkillDesc(state: CharacterState): string {
        ...
        if (skill2 > 0) {
            msg += this.PassiveIntroduceTwo.replace("{skillLv}", skill2 + "")
                .replace("{healVal}", Math.floor(7 * skill2) + "")+ "\n";
        } else {
            msg += this.PassiveIntroduceTwo.replace("{skillLv}", "未开启")
                .replace("{healVal}", Math.floor(7) + "")+ "\n";
        }
        if (skill3 > 0) {
