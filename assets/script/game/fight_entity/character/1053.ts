...
@RegisterCharacter({ id: "1053" })
export class Character extends CharacterMetaState {

    name: string = "杨戬"

    AnimationDir: string = "game/fight_entity/character/1053"


    AvatarPath: string = "game/texture/frames/hero/1053/spriteFrame"

    HeaderPath: string = "game/texture/frames/hero/Header/1053/spriteFrame"
  ...
  AnimationScale: number = 1
   ...
   position = 1
    ...
    顽强战意 Lv{skillLv}
    受到普通攻击时，提高自身生命上限{healVal}点，最多叠加20层
    `.replace(/ /ig, "")

    SkillIntroduce: string = `
    
    哪吒协同 Lv{skillLv}
    与哪吒在同一队伍时，增加自身{healVal}点生命上限，{healVal2}点攻击，{healVal3}点速度
    `.replace(/ /ig, "")

    introduce: string = "二郎神，仪容清俊，身佩三尖两刃刀，身旁哮天犬威风凛凛。"

    skillValue: string = `穿云长枪  顽强战意  哪吒协同`
    ...
    public getSkillDesc(state: CharacterState): string {
        ...
        if (skill2 > 0) {
            msg += this.PassiveIntroduceTwo.replace("{skillLv}", skill2 + "")
                .replace("{healVal}", Math.floor(141 * skill2) + "") + "\n";
        } else {
            msg += this.PassiveIntroduceTwo.replace("{skillLv}", "未开启")
                .replace("{healVal}", Math.floor(141) + "")+ "\n";
        }
        if (skill3 > 0) {
