...
@RegisterCharacter({ id: "1034" })
export class Character extends CharacterMetaState {

    name: string = "南岳大帝"

    AnimationDir: string = "game/fight_entity/character/1034"


    AvatarPath: string = "game/texture/frames/hero/1034/spriteFrame"

    HeaderPath: string = "game/texture/frames/hero/Header/1034/spriteFrame"
    ...
    通灵神箭 Lv{skillLv}
    每回合对同位置敌方造成{healVal}点飞弹伤害
    `.replace(/ /ig, "")

    PassiveIntroduceTwo: string = `
    
    草船借箭 Lv{skillLv}
    受到火焰伤害时，增加自身飞弹伤害{healVal}点，最多叠加99层
    `.replace(/ /ig, "")

    SkillIntroduce: string = `
    
    大帝协同 Lv{skillLv}
    与西岳大帝在同一队伍时，增加自身{healVal}点生命上限，{healVal2}点攻击，{healVal3}点速度
    `.replace(/ /ig, "")

    introduce: string = "衡山大帝，曾帮助大禹治水。"

    skillValue: string = "通灵神箭  草船借箭  大帝协同"
    ...
    public getSkillDesc(state: CharacterState): string {
        ...
        if (skill2 > 0) {
            msg += this.PassiveIntroduceTwo.replace("{skillLv}", skill2 + "")
                .replace("{healVal}", Math.floor(13 * skill2) + "") + "\n";
        } else {
            msg += this.PassiveIntroduceTwo.replace("{skillLv}", "未开启")
                .replace("{healVal}", Math.floor(13) + "") + "\n";
        }
        if (skill3 > 0) {
